...
/*        rs2_set_option(rs2_options(), RS2_OPTION_EMITTER_ON_OFF, 0f, error)
        error.check()*/

        val info = rs2_get_device_info(device, RS2_CAMERA_INFO_SERIAL_NUMBER, error)
        rs2_config_enable_device(config, info, error)

        rs2_config_enable_stream(config, RS2_STREAM_INFRARED, 1, depthWidth, depthHeight, RS2_FORMAT_Y8, depthFps, error)
        error.check()
...
class RS2Sensor(
    ...
    override fun waitForFrames() {
        ...
        for (i in 0 until frameCount) {
            ...
            error.check()
            val cmp = rs2_is_frame_extendable_to(frame, RS2_EXTENSION_VIDEO_FRAME, error)
            if (cmp != 0) {
